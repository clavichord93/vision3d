...
at::Tensor gather_by_index(at::Tensor points, at::Tensor idx) {
  ...
  gather_by_index_kernel_launcher(
      ...
      points.data_ptr<float>(), idx.data_ptr<long>(), output.data_ptr<float>());

  return output;
}

at::Tensor gather_by_index_grad(at::Tensor grad_out, at::Tensor idx, const int n) {
  ...
  gather_by_index_grad_kernel_launcher(grad_out.size(0), grad_out.size(1), n,
                                       ...
                                       idx.data_ptr<long>(), output.data_ptr<float>());
...
at::Tensor group_gather_by_index(at::Tensor points, at::Tensor idx) {
  ...
  group_gather_by_index_kernel_launcher(
...
at::Tensor group_gather_by_index_grad(at::Tensor grad_out, at::Tensor idx,
                                      const int n) {
  CHECK_INPUT(grad_out);
  CHECK_IS_FLOAT(grad_out);
  CHECK_INPUT(idx);
  ...
  group_gather_by_index_grad_kernel_launcher(
      grad_out.size(0), grad_out.size(1), n, idx.size(1), idx.size(2),
      grad_out.data_ptr<float>(), idx.data_ptr<long>(), output.data_ptr<float>());
